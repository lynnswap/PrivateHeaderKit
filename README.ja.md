...
output base には、stable な source-label path、immutable generation、source ごとの
SQLite state database を配置します。
...
`<output-base>/<source-label>/` を使い、publication metadata と immutable
...
旧 `.state/<source-label>/manifest.json` と `runs/` の data は resumable state として
...
実 directory として存在する `<output-base>/<source-label>/` は黙って取り込みません。
...
`.privateheaderkit/<source-label>/legacy-backups/` 以下に保持します。legacy tree を検証
